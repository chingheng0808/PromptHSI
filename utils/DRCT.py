...
class RDG_TPAB(nn.Module):
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, shift_size, mlp_ratio, qkv_bias, qk_scale, drop, attn_drop, drop_path, norm_layer, gc, patch_size, img_size):
        super(RDG_TPAB, self).__init__()

        self.swin1 = SwinTransformerBlock_TPAB(dim=dim, input_resolution=input_resolution,
                                          num_heads=num_heads, window_size=window_size,
                                          shift_size=0,  # For first block
                                          mlp_ratio=mlp_ratio,
                                          qkv_bias=qkv_bias, qk_scale=qk_scale,
        ...
        self.swin2 = SwinTransformerBlock_TPAB(dim + gc, input_resolution=input_resolution,
                                          num_heads=num_heads - ((dim + gc)%num_heads), window_size=window_size,
                                          shift_size=window_size//2,  # For first block
                                          mlp_ratio=mlp_ratio,
                                          qkv_bias=qkv_bias, qk_scale=qk_scale,
        ...
        self.swin3 = SwinTransformerBlock_TPAB(dim + 2 * gc, input_resolution=input_resolution,
                                          num_heads=num_heads - ((dim + 2 * gc)%num_heads), window_size=window_size,
                                          shift_size=0,  # For first block
                                          mlp_ratio=mlp_ratio,
                                          qkv_bias=qkv_bias, qk_scale=qk_scale,
        ...
        self.swin4 = SwinTransformerBlock_TPAB(dim + 3 * gc, input_resolution=input_resolution,
                                          num_heads=num_heads - ((dim + 3 * gc)%num_heads), window_size=window_size,
                                          shift_size=window_size//2,  # For first block
                                          mlp_ratio=1,
                                          qkv_bias=qkv_bias, qk_scale=qk_scale,
        ...
        self.swin5 = SwinTransformerBlock_TPAB(dim + 4 * gc, input_resolution=input_resolution,
                                          num_heads=num_heads - ((dim + 4 * gc)%num_heads), window_size=window_size,
                                          shift_size=0,  # For first block
                                          mlp_ratio=1,
                                          qkv_bias=qkv_bias, qk_scale=qk_scale,
    ...
    def forward(self, x, xsize, text_embed):
        x1 = self.pe(self.lrelu(self.adjust1(self.pue(self.swin1(x, xsize, text_embed), xsize))))
        x2 = self.pe(self.lrelu(self.adjust2(self.pue(self.swin2(torch.cat((x, x1), -1), xsize, text_embed), xsize))))
        x3 = self.pe(self.lrelu(self.adjust3(self.pue(self.swin3(torch.cat((x, x1, x2), -1), xsize, text_embed), xsize))))
        x4 = self.pe(self.lrelu(self.adjust4(self.pue(self.swin4(torch.cat((x, x1, x2, x3), -1), xsize, text_embed), xsize))))
        x5 = self.pe(           self.adjust5(self.pue(self.swin5(torch.cat((x, x1, x2, x3, x4), -1), xsize, text_embed), xsize)))
        

        return x5 * 0.2 + x
    
class SwinTransformerBlock_TPAB(nn.Module):
    r""" Swin Transformer Block.
    Args:
        dim (int): Number of input channels.
        input_resolution (tuple[int]): Input resulotion.
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super(SwinTransformerBlock_TPAB, self).__init__()
        self.dim = dim
        self.input_resolution = input_resolution
        self.num_heads = num_heads
        self.window_size = window_size
        ...
        # mlp_hidden_dim = int(dim * mlp_ratio)
        # self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
        self.mlp = TPAB(in_dim=dim, act=act_layer, bias=True)
    ...
    def forward(self, x, x_size, text_embed):
        H, W = x_size
        B, L, C = x.shape
        # assert L == H * W, "input feature has wrong size"
        ...
        # FFN
        x = shortcut + self.drop_path(x)
        x = x + self.drop_path(self.mlp(self.norm2(x), text_embed, x_size))
...
class TPAB(nn.Module):
    def __init__(self, in_dim, act=nn.ReLU, bias=False):
        super(TPAB, self).__init__()
        self.in_dim = in_dim
        
        self.gPool = nn.AdaptiveAvgPool1d(output_size=1)
        self.linear_text = nn.Linear(512, in_dim, bias=bias)
        self.sigma = nn.Parameter(torch.ones(1, 1))
        self.linear_qkv = nn.Linear(in_dim, 3*in_dim, bias=bias)
        self.linear_att = nn.Linear(in_dim, in_dim, bias=bias)
        self.linear_out = nn.Linear(in_dim, in_dim, bias=bias)
        self.act = act()
        self.depthwise_conv = nn.Sequential(nn.Conv2d(in_dim, in_dim, kernel_size=5, stride=1, padding=2, dilation=1, groups=in_dim), act())

    def forward(self, x, text_embed, x_size):
        text_embed = self.linear_text(text_embed) # (B 1 dim)
        x = self.gPool(x) @ text_embed # (B x_size[0]*x_size[1] dim)
        x = self.linear_qkv(x)
        ...
        x = (k.transpose(-2, -1) @ q) * self.sigma
        x = x.softmax(dim=-1)
        x = self.linear_att(v @ x)
        
        x = self.act(x)
        res = x
        x = x.transpose(1, 2).view(x.shape[0], self.in_dim, x_size[0], x_size[1])  # b Ph*Pw c
